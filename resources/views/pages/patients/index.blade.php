            ...
            <div class="overflow-hidden shadow">
                <table class="min-w-full divide-y divide-gray-200 table-fixed dark:divide-gray-600">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        @foreach ($records['patients'] as $n => $patient)
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                ...
                                <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                    <div class="text-sm font-normal text-gray-500 dark:text-gray-400">
                                        <div class="text-base font-semibold dark:text-white">{{ $patient->ptt_fname.' '.$patient->ptt_lname }}</div>
                                        <div class="text-sm font-normal text-gray-500 dark:text-gray-400">{{ $patient->ptt_email }}</div>
                                    </div>
                                </td>
                                <td class="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">
                                    @if ($patient->rl_active)
                                        Yes
                                    @else
                                        No
                                    @endif
                                ...
                                <td class="p-4 space-x-2 whitespace-nowrap">
                                    ...
                                    <button type="button" data-drawer-target="drawer-update-default" data-drawer-show="drawer-update-default" aria-controls="drawer-update-default" data-drawer-placement="right" class="patient-form inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800" value="{{ md5($patient->ptt_id) }}">
                                        <i class="fa fa-edit"></i>
                                    </button>
                                    @endif
                                    @if ($user->menus[$s_menu]['can']['delete'])
                                    <button type="button" data-drawer-target="drawer-delete-default" data-drawer-show="drawer-delete-default" aria-controls="drawer-delete-default" data-drawer-placement="right" class="delete inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-700 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900"  value="{{ md5($patient->ptt_id) }}">
...
<div id="drawer-delete-default" class="fixed top-0 right-0 z-[45] w-full h-screen max-w-xs p-4 overflow-y-auto transition-transform translate-x-full bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-label" aria-hidden="true">
    ...
    <form action="{{ route('ptt.delete') }}" method="post">
        @csrf
...
@include('pages.patients.add')
@include('pages.patients.edit')
